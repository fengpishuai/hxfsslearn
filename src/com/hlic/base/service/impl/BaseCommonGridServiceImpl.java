...
import com.hlic.base.dao.mapper.DofDctColsMapper;
import com.hlic.base.dao.mapper.DofDictsMapper;
import com.hlic.base.dao.mapper.DofStoreColsMapper;
import com.hlic.base.dao.mapper.DofStoresMapper;
import com.hlic.base.dao.mapper.SysSqlFilterMapper;
import com.hlic.base.dao.mapper.WdRptqueryConfMapper;
import com.hlic.base.dao.mapper.WdRptqueryFilterMapper;
...
public class BaseCommonGridServiceImpl implements BaseCommonGridService {
	...
	public String queryUniqueKeyValue(String inputJson) {
		...
		if(dofDictsList.size() > 0){
			...
			}else if(!"".equals(dofDicts.getDctId())){
				//1、自增
				sqlMap = new HashMap<String, Object>();
				String sqlStr = "select count(*) from "+tId;
				sqlMap.put("sql", sqlStr);
				//sqlMap.put("table", tId);
				int increment = baseCommonGridMapper.queryCount(sqlMap);
				int num = (Integer) increment;
			}
