...
**********************************************************/
    ...
    function createGrid(){
    	...
    	dom.hlicStoGrid({
            ...
            "colModel" : [
                ...
                {name : 'COL_TITLE',index : 'colTitle',width : 170,editable: true},
                ...
                {name : 'STO_KEY_FIELD_FCB',index : 'STO_KEY_FIELD_FCB',width : 78,editable: true,edittype:'checkbox',formatter:'checkbox',align:"center"},
  ...
  //定义普通select的edittype+++++++开始+++++++++
    ...
    function initGrid(stoId){
    	...
    	$.ajax({
            url:sys_ctx+"/base/queryCreateStoreColsList.action",
            type:'post',
            dataType:'json',
        	...
        	if(data.colList.length > 0){
        		...
        		for ( var i = 0; i < data.colList.length; i++){
        			if(data.colList[i].hasOwnProperty("STO_KEY_FIELD")){
        				data.colList[i].STO_KEY_FIELD = 1;
    ...
    function querydofstores(stoOwnerBa){
    	var queryDatas = hlicGridPlus.queryTableRows("queryBase/queryDofStoresList.action",{"stoOwnerBa":stoOwnerBa});
     	if(queryDatas.length > 0){
    		...
    		if(queryDatas[0].stoId != "" && queryDatas[0].stoId != undefined){
    ...
    function addTable(){
    	$(window.top.document).find("#addDo").hlicComSelect({
            ...
            "callback" : function(data){
            	doSaveData.stoId = data.F_ID;
            	doSaveData.stoName = data.F_MC;
    ...
    //删除新表
    function delTable(){
    	var stoId = $(".item-select").attr("stoid");
    	if(stoId == undefined){
    ...
    function addRow(){
    	var sendData = {};
    	if(doSaveData.stoId == "" || doSaveData.stoId == undefined){
    		tipInfo("提示","请选择一个对象！",1000,0);
    		return;
    	}
    	sendData.STO_ID = doSaveData.stoId;
