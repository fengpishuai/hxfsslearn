...
function showAsk(){
	...
	saveData.pkey = pkey;
	$.ajax({ 
        url:"${ctx}/ypml/queryHxQuertionMxPkey.action", 
        type:'post', 
        dataType:'json', 
        ...
        success : function(data){ 
        	if(data.length > 0){
        		$(".ke-edit-iframe").contents().find("body").html(data[0].fHdnr);
        		ssdjbh = data[0].fSsdjbh;
        	}
        }
	});
}
...
function save(){
	...
	if(submitFlag){
		...
		saveData.ssdjbh = ssdjbh;
		$.ajax({ 
	        url:"${ctx}/ypml/updateHxQuestionMx.action", 
	        type:'post', 
	        dataType:'json', 
	        contentType:"application/json",
	        data:JSON.stringify(saveData),
